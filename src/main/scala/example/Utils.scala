...
// https://raw.githubusercontent.com/ThoughtWorksInc/DeepLearning.scala-website/v1.0.0-doc/ipynbs/Utils.sc
object Utils {
  /**
  ...
  def makeVectorized(ndArray: INDArray, numberOfClasses: Int): INDArray = {
    ...
    // println("The shape of input ndArray is " + shape.toList)

    val p = Nd4j.zeros(shape(0), numberOfClasses)    
    // println("The shape of vectorized p is " + p.shape().toList)

    for (i <- 0 until shape(0)) {
      val double = ndArray.getDouble(i, 0)
        // println("The value obtained at the specified index is  " + double)
